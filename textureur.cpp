...
void Textureur::init(DescripteurTache &tache)
{
    ...
    if (utiliserPermuteur)
        permuteur = new Permuteur(nb_bloc);
                        ...
                        Image_4b &dest, int dest_x, int dest_y)
{
    ...
    */
    if (src.checkdim(src_x, src_y, "win2win-src-1") && src.checkdim(src_x + w - 1, src_y + h - 1, "win2win-src-2") && dest.checkdim(dest_x, dest_y, "win2win-dest-1") && dest.checkdim(dest_x + w - 1, dest_y + h - 1, "win2win-dest-2"))
    {
