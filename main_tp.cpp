...
int main(int argc, char **argv)
{
    ...
    if (argc >= 2)
        tache.fichierImage = argv[1];
    if (argc >= 3)
        algo = atoi(argv[2]);
    if (argc >= 4)
        tache.racineNombreBlocs = atoi(argv[3]);
    if (argc >= 5)
        tache.recouvrement = atoi(argv[4]);
    if (argc >= 6)
        tache.largeurSortie = atoi(argv[5]);
    if (argc >= 7)
        tache.hauteurSortie = atoi(argv[6]);
    if (argc > 7)
        fprintf(stderr, "on ignore les arguments Unix au dela du 6eme\n");
