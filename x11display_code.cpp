...
void TTrueColor24Open(const char *name)
{
  ...
  if (depth != 24)
    fprintf(stderr, "running with screendepth %d\n", depth);
}
...
void X11Display::ClickLine(int f)
{
  ...
  if (f)
    XSelectInput(dpy, win, ButtonPressMask | ButtonReleaseMask | ButtonMotionMask | PointerMotionHintMask);
  ...
  // handle ButtonMotion
  {
    while (XCheckMaskEvent(dpy, ButtonMotionMask, &evt))
      ; // skip pending motions
